from app.db.repository import MongoRepository
import httpx
from fastapi import HTTPException
from app.config import BASE_URL
from app.models.languages import Languages

class LanguagesService:
    def __init__(self, db):
        self.db = db
        self.repo = MongoRepository(self.db)
        self.base_url = BASE_URL

    async def fetch_languages(self)-> list[Languages]:
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(f"{self.base_url}/languages")
                response.raise_for_status()
                data = response.json()

                if not data:
                    raise HTTPException(status_code=500, detail="No languages found")

                language_data = data.get("results", [])
                return [Languages.from_dict(language) for language in language_data]
        except HTTPException as e:
            raise HTTPException(status_code=502, detail=f"Error in accessing external API: {str(e)}")

    async def populate_languages(self)->list[Languages]:
        try:
            items = await self.fetch_languages()

            await self.repo.save_items_if_not_exist(
                items = items,
                collection_name = "languages",
                unique_field="index"
            )
            return items
        except HTTPException as e:
            raise HTTPException(status_code=502, detail=f"Error in saving items: {str(e)}")

    async def get_languages(self)->list[Languages]:
        try:
            await self.populate_languages()

            collection = self.db["languages"]
            cursor = collection.find({})
            results = [doc async for doc in cursor]

            return [Languages.from_dict(doc) for doc in results]
        except HTTPException as e:
            raise HTTPException(status_code=500, detail=f"Error reading from DB: {str(e)}")